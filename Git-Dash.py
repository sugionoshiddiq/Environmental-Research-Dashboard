...
if "Affiliations" in filtered.columns:
    ...
    affs_counts.columns = ["Collaborator", "Count"]  # ganti nama kolom
...
if "Journal Name" in filtered.columns:
    ...
    journal_counts.columns = ["Journal/Conference", "Count"]  # ganti nama kolom
    journal_counts = journal_counts.sort_values(by="Count", ascending=True)

    fig_c = px.bar(
        journal_counts,
        y="Journal/Conference",
        x="Count",
        text="Count",
        orientation="h",
        title="Top 10 Journals/Conferences",
        hover_name="Journal/Conference",
        hover_data={"Count": True}
    )
